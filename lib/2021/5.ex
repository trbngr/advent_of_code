defmodule Y2021.D5 do
  ...
  def p1, do: p1(input_stream() |> Enum.to_list())
  def p1(input), do: run_input(input)

  def p2, do: p2(input_stream() |> Enum.to_list())
  def p2(input), do: run_input(input, :inc_diagonals)
  ...
  defp mark_line([{x1, y}, {x2, y}], grid, _opt),
    do: Enum.reduce(x1..x2, grid, &mark_point({&1, y}, &2))

  defp mark_line([{x, y1}, {x, y2}], grid, _opt),
